...
function processSavedPronunData(savedResponses){
    ...
    savedResponses.forEach(savedResponse => {
        ...
        response.forEach(pronunErrorObj => {
            ...
            $word.attr('position', `${pronunErrorObj.errorPostion}`);
        ...
        // Check if there are any elements with the class 'pronun_error'
        const $lastPronunError = $pronunErrorsWrap.find('.pronun_error').last();
        console.log("Last .pronun_error element found: ", $lastPronunError);

        // Check if the element exists
        if ($lastPronunError.length > 0) {
            // Extract the ID of the last element
            const lastPronunErrorId = $lastPronunError.attr('id');
            console.log("ID of the last .pronun_error element: ", lastPronunErrorId);
            
            // Use regex to extract the number from the ID
            const match = lastPronunErrorId.match(/SIDEPANEL_PRONUN_ERROR_(\d+)/);
            console.log("Regex match result: ", match);
            
            if (match) {
                // Assign the number to pronunErrorCounter
                pronunErrorCounter = parseInt(match[1], 10);
                console.log("Assigned pronunErrorCounter: ", pronunErrorCounter);
            } else {
                console.log("No match found in the ID.");
            }
        } else {
            console.log("No .pronun_error elements found.");
        }

        console.log("Final pronunErrorCounter value: ", pronunErrorCounter);
    });
}
...
function getWordPosition($word) {
    // Get the full text of the parent paragraph
    let parent = $word.closest('p.transcript-text');
    
    // Retrieve the 'start' attribute from the current word for unique identification
    let targetStart = $word.data('start');
    
    // Initialize position tracking variables
    let position = 0;
    let wordFound = false;
    
    // Iterate over all child elements in the parent paragraph
    parent.contents().each(function() {
        // If this is a text node, add its length to the position tracker
        if (this.nodeType === Node.TEXT_NODE) {
            position += this.textContent.length;
        }
        
        // If this is a span element
        else if (jQuery(this).is('span')) {
            // Get the 'start' attribute of the span
            let spanStart = jQuery(this).data('start');

            // Check if this span is the word we are looking for
            if (spanStart === targetStart) {
                wordFound = true;
                return false; // Break the loop when we find the word
            } else {
                // If not the target span, add the span's text length (with space) to position
                position += jQuery(this).text().length + 1; // 1 for space after span
            }
        }
    });

    // Return the final position of the word inside the parent element
    if (wordFound) {
        return position;
    } else {
        // Return -1 if the word was not found (for debugging)
        return -1;
    }
}
...
async function markPronunError($word,wrongPhonetics = null, manuallyMarked = true) {
    ...
    try {
        ...
        console.log(pronunErrorCounter)
        // Add the pronun-error class and set the ID
        $word.addClass('pronun-error');
        $word.attr('id', `PRONUN_ERROR_${pronunErrorCounter}`);
        
        // Find the position of the word within the parent's text
        let position = getWordPosition($word);
        $word.attr('position', `${position}`);
...
function addPronunError(pronunErrorObj){
    ...
    const { start, end, text, correctPronunAudio, correctPhonetic,fileIndex,errorId, wrongPhonetics, errorPostion } = pronunErrorObj;
...
function adjustWrapperDataHeight(){
    let maxHeight = window.innerHeight - 300;
    let suggestionsParentWrap = document.querySelector('.suggestions-parent-wrap');
    let activeMistakeContainer = document.querySelector('#result-data-carousel .swiper .swiper-slide-active .mistake-container');
    setTimeout(function(){
        suggestionsParentWrap.scrollTop = suggestionsParentWrap.scrollHeight ;
    },250)
